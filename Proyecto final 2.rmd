...
df_final= df%>% mutate(tipo_habitacion=as.factor(tipo_habitacion),
                ...
                #  grupo_habitacion=ifelse(habitaciones>=0 &&          habitaciones<3,'Chica',ifelse(habitaciones>=3 && habitaciones<6,'Mediana','Grande'))
                 , grupo_habitacion=cut(habitaciones,c(0,2,5,Inf))
             )

df_final$grupo_habitacion
...
df_final%>% ggplot() + geom_histogram(aes(x=log(precio_euros)))
...
mod0 <- lm(log(precio_euros) ~  distritos + tipo_habitacion+personas+ banios+ habitaciones+camas+estancia_min+puntuacion+TV+Wifi+Air_conditioning+Elevator+Breakfast+Pets_allowed+Pool+Patio_or_balcony+check_in_24_hs+Smart_lock, data=df_final)
