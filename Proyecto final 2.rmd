...
```{r fig.cap="QQ-Plot de los Residuos Studentizados",results='asis', fig.align='center'}
...
bp=data.frame(ks.test(df_final$residuos_int, 'pnorm')$p.value)
...
```{r results='asis',tab.cap="Métricas asociadas al Modelo",echo=FALSE,warning=FALSE,message=FALSE}
tab=data.frame( Varianza_Mod0=summary(mod0)$sigma,
                Varianza_Mod1=summary(mod1)$sigma,
...
tab=data.frame( Varianza_Mod0=summary(mod0)$sigma,
                Varianza_Mod1=summary(mod1)$sigma,
                Varianza_Mod2=summary(mod2)$sigma,
                R2_Mod0=summary(mod0)$r.squared,
                R2_Mod1=summary(mod1)$r.squared,
...
res_ext<-rstudent(mod2) #residuos
res_int<-rstandard(mod2) #residuos
yhat<-fitted(mod2) #ygorro
#grafico

df_3$predichos <- yhat #agrego los residuos a la tabla
df_3$residuos <- res_ext #agrego los ygorro a la tabla
df_3$residuos_int <- res_int #agrego los ygorro a la tabla

ggplot(df_3, aes(x = predichos , y = residuos)) +
  geom_point(size=1) +  geom_hline(yintercept = 0) + labs(y="Residuos",x="Predichos") +theme_bw() 

breusch_pagan(mod2)

ggplot() + geom_histogram(aes(x=rstudent(mod2)),bins = 100) + xlab("Residuos Studentizados")+  theme_bw() + theme (axis.title.y = element_blank())
...
Interpretando algunos de los coeficientes personas puntuacion
